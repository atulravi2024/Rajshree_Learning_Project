...
# Multiple manifest targets with their respective depths from project root
MANIFEST_TARGETS = [
    {
        "path": os.path.join(PROJECT_ROOT, "memory", "06 Internal Audit", "v2.0", "Mockups", "assets", "asset_manifest.js"),
        "depth": 5
    },
    {
        "path": os.path.join(PROJECT_ROOT, "memory", "06 Internal Audit", "v2.0", "assets", "asset_manifest.js"),
        "depth": 4
    }
]
...
def scan_project():
    # We store the raw relative path and base metadata here
    items = []
    ...
    for root, dirs, files in os.walk(PROJECT_ROOT):
        ...
        for file in files:
            ...
            items.append({
                ...
                "date": mod_date,
                "rel_path": rel_path.replace('\\', '/'), # Standardized slashes
                "risk": risk,
                "compliance": 1,
...
window.PROJECT_ASSETS = """
    ...
    for target in MANIFEST_TARGETS:
        ...
        # Build the final asset list for this target
        target_assets = []
        for item in items:
            asset = item.copy()
            # Calculate path relative to THIS manifest location
            asset["path"] = prefix + asset.pop("rel_path")
        ...
        with open(target_path, 'w', encoding='utf-8') as f:
            f.write(header)
            json.dump(target_assets, f, indent=4)
            f.write(";\n")
        
        print(f"Manifest written to: {target_path}")

    print(f"Total assets processed: {len(items)}")
